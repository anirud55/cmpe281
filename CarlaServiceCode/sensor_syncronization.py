...
def main_run():
    ...
    try:
        ...
        cam01.listen(lambda data: sensor_callback(data, sensor_queue, "temperature: 65F"))
        sensor_list.append(cam01)

        lidar_bp.set_attribute('points_per_second', '100000')        
        lidar01 = world.spawn_actor(lidar_bp, carla.Transform())
        lidar01.listen(lambda data: sensor_callback(data, sensor_queue, "Tire Pressure T1=36 T2=33 T3=32 T4=34"))
        sensor_list.append(lidar01)

        lidar_bp.set_attribute('points_per_second', '1000000')        
        lidar02 = world.spawn_actor(lidar_bp, carla.Transform())
        lidar02.listen(lambda data: sensor_callback(data, sensor_queue, "Humidity: 60%"))
        ...
        while True:
            ...
            client = MongoClient("mongodb+srv://admin:cmpe281@cmpe281.9y980.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
            sensor_db = client["sensor_frame_data"]
            # collection = sensor_frame_db.test_collection
            # print(myclient.list_database_names())
            sensor_collection = sensor_db["frame"]
            ...
            try:
                for i in range (0, len(sensor_list)):
                    ...
                    post["frame"].append(s_frame[0])
                    post["sensor"].append(s_frame[1])
                print("post: ", post)
